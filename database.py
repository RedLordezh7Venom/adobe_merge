...
# Get table schema
def get_table_schema(db_name, user, password, table_name='', host='localhost', port='5432'):
    conn = psycopg2.connect(dbname=db_name, user=user, password=password, host=host, port=port)
    cursor = conn.cursor()
...
if __name__ == "__main__":
    ...
    print(list_all_tables(db_name, user, password))
    tables = list_all_tables(db_name, user, password)
    
    # Build schema description
    schema_description = "Database Schema:\n"
    for table in tables:
        table_schema = get_table_schema(db_name, user, password, table)
